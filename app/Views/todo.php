...
<div class="rounded-md border p-3 space-y-3">
    ...
    <form method="post" action="/tambah" class="space-y-2">
        ...
        <div class="text-right">
            <button
                    type="submit"
                    class="px-2 py-1 rounded bg-gray-600 hover:bg-gray-600/80 text-white"
            >
                Simpan
            </button>
        </div>
    ...
    <?php foreach ($todo as $t): ?>
        <li class="flex items-center justify-between gap-2">
            <form method="post" action="/edit/<?= $t['id'] ?>" class="flex-1 flex items-center justify-between gap-4">
                ...
                <input type="text" name="nama" class="flex-1 line-clamp-3 peer-checked/done:line-through peer-checked/done:opacity-80" value="<?= $t['nama'] ?>"/>
